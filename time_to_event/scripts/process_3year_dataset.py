#!/usr/bin/env python3
"""
Process 3-year dataset for time-to-event (survival analysis) models.
"""
...
from src.data_loader import load_3year_dataset, validate_temporal_data
...
def main():
    print("=" * 80)
    print("PROCESSING 3-YEAR DATASET FOR TIME-TO-EVENT MODELS")
    print("=" * 80)
    
    # Paths
    script_dir = os.path.dirname(os.path.abspath(__file__))
    base_dir = Path(script_dir).parent
    data_dir = base_dir / "data" / "processed_3year"
    ...
    raw_data_path = str(project_root / "data" / "CNUH_3Y.csv")
    ...
    print("-" * 80)
    df = load_3year_dataset(raw_data_path, impute_event_time=True)
    ...
    with open(results_dir / "processing_report_3year.json", 'w') as f:
        json.dump(report, f, indent=2, default=str)
    print(f"Saved processing report to: {results_dir / 'processing_report_3year.json'}")
